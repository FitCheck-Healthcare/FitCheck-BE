...
exports.postConsultRequest = postConsultRequest;
exports.getMyConsultRequests = getMyConsultRequests;
exports.getConsultRequest = getConsultRequest;
const consultRequest_js_1 = require("../types/consultRequest.js");
const consultRequests_service_js_1 = require("../services/consultRequests.service.js");
...
function parseCreateBody(body) {
    ...
    const preferredDate = typeof raw.preferredDate === 'string' ? raw.preferredDate.trim() : '';
    const preferredTime = typeof raw.preferredTime === 'string' ? raw.preferredTime.trim() : '';
    const topic = typeof raw.topic === 'string' ? raw.topic.trim() : '';
    const topicDetail = typeof raw.topicDetail === 'string' ? raw.topicDetail.trim() : '';
...
async function postConsultRequest(req, res) {
    try {
        const parsed = parseCreateBody(req.body);
        if (typeof parsed === 'string') {
            return (0, response_js_1.sendError)(res, 400, 'VALIDATION_ERROR', parsed);
        }
        const created = await (0, consultRequests_service_js_1.createConsultRequest)(parsed, req.userId);
        ...
        if (message === 'TRAINER_NOT_FOUND') {
            return (0, response_js_1.sendError)(res, 404, 'NOT_FOUND', '트레이너를 찾을 수 없습니다.');
        }
        if (message === 'TRAINER_GYM_MISMATCH') {
            return (0, response_js_1.sendError)(res, 400, 'VALIDATION_ERROR', '트레이너가 해당 헬스장 소속이 아닙니다.');
        }
        return (0, response_js_1.sendError)(res, 500, 'INTERNAL_ERROR', message);
    }
}
async function getMyConsultRequests(req, res) {
    try {
        ...
        const statusRaw = typeof req.query.status === 'string' && req.query.status.trim()
            ...
            : undefined;
        const status = statusRaw && (0, consultRequests_service_js_1.isConsultRequestStatus)(statusRaw) ? statusRaw : undefined;
        if (statusRaw && !status) {
            return (0, response_js_1.sendError)(res, 400, 'VALIDATION_ERROR', '유효하지 않은 status 값입니다.');
        }
        const page = parsePositiveInt(req.query.page, 1);
        const limit = parsePositiveInt(req.query.limit, 20, 50);
        const result = await (0, consultRequests_service_js_1.listMyConsultRequests)(userId, { status, page, limit });
...
async function getConsultRequest(req, res) {
    try {
        const userId = req.userId;
        if (!userId) {
            return (0, response_js_1.sendError)(res, 401, 'UNAUTHORIZED', '인증이 필요합니다.');
        ...
        const item = await (0, consultRequests_service_js_1.getConsultRequestByIdForUser)(userId, id);
        if (!item) {
            return (0, response_js_1.sendError)(res, 404, 'NOT_FOUND', '상담 신청을 찾을 수 없습니다.');
        }
        return (0, response_js_1.sendSuccess)(res, item);
    }
    catch (err) {
        const message = err instanceof Error ? err.message : '서버 오류가 발생했습니다.';
        return (0, response_js_1.sendError)(res, 500, 'INTERNAL_ERROR', message);
